...
PROMPT_TEMPLATE = """Edit this image. It is a product photo of a 50 ml perfume bottle from One Bold Chemist. Keep the bottle EXACTLY as it is: the same glass bottle shape, the same brushed silver cap, the same liquid colour, and the same label with the exact text "{label}", "extrait de parfum" and "one bold chemist" plus the same small halftone illustration inside the label's frame. Do not redraw, retouch, move, rescale or rotate the bottle, and do not change a single letter or the artwork on the label.

Add a photorealistic still life of the fragrance's raw ingredients arranged in a loose, natural cluster around the base of the bottle on the same white surface, some pieces slightly in front of the bottle and some behind it, so the bottle stays the hero in the centre:
{ingredients}

Every ingredient is a real, physical, tactile object at true scale relative to a 50 ml bottle, with realistic textures and soft natural shadows on the white ground. The ingredients touch and overlap slightly, like an editorial perfume-ingredient still life shot from a slightly elevated front angle that matches the bottle's perspective. Pure seamless white background with soft, even studio lighting. No text, no captions, no extra props, no hands, no reflections of other objects. Square 1:1 image, high-end product photography."""
